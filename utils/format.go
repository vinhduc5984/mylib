...
"#.###,###### => 12.345,678900
*/
func FormatNumber(format string, number float64) string {
	...
	if strings.TrimSpace(format) != "" {
		...
		if len(formatSplit) == 2 {
			fracStr = formatSplit[1]
		}
	}
	if number == 0 || math.Mod(number*2, 1) == 0 && !strings.Contains(fracStr, "0") {
		l := fmt.Sprintf("%d", len(fracStr))
		fc := "%0." + l + "f"
		return fmt.Sprintf(fc, number)
	}
	s, e := renderFloat(format, number)
	if e != nil {
		s, _ = renderFloat(FORMAT_NUM, number)
	}
	return s
}
		...
		if len(formatDirectiveIndices) > 0 {
			...
			if formatDirectiveIndices[0] == 0 {
				if formatDirectiveChars[formatDirectiveIndices[0]] != '+' {
					...
					return "", fmt.Errorf("RenderFloat(): invalid positive sign directive")
				}
				positiveStr = "+"
			...
			if len(formatDirectiveIndices) == 2 {
				if (formatDirectiveIndices[1] - formatDirectiveIndices[0]) != 4 {
					return "", fmt.Errorf("RenderFloat(): thousands separator directive must be followed by 3 digit-specifiers")
				}
